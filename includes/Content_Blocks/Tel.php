<?php
/**
 * Content Block to render the Click to Call metabox.
 *
 * @since      1.0
 *
...
final class Tel extends Content_Block {

	/**
	 * @since 1.0
	 * @var string
	 */
	const ID = 'click_to_call';
	...
	public function __construct( string $id ) {
		...
		$atts = array(
			'name'                => __( 'Click to Call', 'connections_call_sms' ),
			'register_option'     => true,
			'permission_callback' => array( $this, 'permission' ),
			'heading'             => __( 'Click to Call', 'connections_call_sms' ),
	...
	public function content() {
		...
		$number = (string) $entry->getMeta(
			array(
				'key'    => '_click_to_call_phone_number',
				'single' => true,
			)
		);
		...
		$text = $entry->getMeta(
			array(
				'key'    => '_click_to_call_text',
				'single' => true,
			)
		);

		echo Plugin::getInstance()->generateTelLink( $number, $text );
	}
}
